...
$(document).ready(function () {
    ...
    $(".add-hours").on("click", function () {
        var hourscontent =
            ...
            '<input readonly class="timeslots form-control" value="09:00" name="start_time[]" />' +
            "</div>" +
            "</div>" +
            '<div class="col-12 col-md-6">' +
            '<div class="form-group">' +
            "<label>End Time</label>" +
            '<input readonly class="timeslots form-control" value="16:00" name="end_time[]" />' +
            "</div>" +
            "</div>" +
            "</div>" +
            "</div>" +
        ...
        $(".tBody").append(
            ...
            <div class="d-flex mb-4 trCopy">
                <div class="col-md-3">
                    ...
                    <div>
                        <select name="medicines_id[]" class="select2 medicines-select">
                ...
                <div class="col-md-2">
                    ...
                    <input type="text" name="dosages[]" class="form-control" id="exampleInputPassword1" placeholder="dosage">
                </div>
                <div class="col-md-3">
                    <label for="exampleInputPassword1">DOSE DURATION:</label>
                    <div>
                ...
                <div class="col-md-3">
                    <label for="exampleInputPassword1">DOSE INTERVAL:</label>
                    <div>
                        <select name="dose_intervals[]" class="select2">
                                <option selected value="daily morning">Daily morning</option>
                                <option value="daily morning and evening">Daily morning and evening</option>
                                <option value="daily morning, noon, and evening">Daily morning, noon, and evening</option>
                ...
                <div class="col-md-3">
                    ...
                    <textarea class="form-control" name="comments[]" id="exampleFormControlTextarea1" rows="5" placeholder="Comment"></textarea>
                </div>
                <div class="col-md-1 d-flex flex-column justify-content-around mt-4">
                    <a class="deleteBtn btn btn-danger px-2"><i class="fa-solid fa-trash text-white" style="font-size: 15px;"></i></a>
                </div>
            </div>
            `
        );

        $('.medicines-select').each(function() {
            const $select = $(this);
            medicines.forEach(function(medicine) {
                $select.append(new Option(medicine.name, medicine.id));
            });
        });
    ...
    // Append options to the newly added select element
$('.medicines-select').each(function() {
    const $select = $(this);
    medicines.forEach(function(medicine) {
        $select.append(new Option(medicine, medicine));
    });
});
...
$(".add-moreInfo").on("click", function () {
    var append =
        '<div class="moreInfoDiv"><label class="col-form-label">Title</label>' +
        '<div class="form-group">' +
        ...
        success: function (result) {
            ...
            JSON.parse(result.data.period_list).forEach((element, index) => {
                $(".display_timing").append(
                    '<div class="row form-row hours-cont delete' +
                        index +
                        '">' +
                        '<div class="col-12 col-md-10">' +
                        '<div class="row form-row">' +
                        ...
                        '<div class="form-group">' +
                        "<label>Start Time</label>" +
                        '<input readonly class="timeslots form-control start_time" name="start_time[]" value="' +
                        element.start_time +
                        '" />' +
                        "</div>" +
                        "</div>" +
                        ...
                        '<div class="form-group">' +
                        "<label>End Time</label>" +
                        '<input readonly class="timeslots form-control end_time" name="end_time[]" value="' +
                        element.end_time +
                        '"/>' +
                        "</div>" +
                        "</div>" +
...
function edit_pharmacy_timeslot() {
    ...
    $.ajax({
        ...
        success: function (result) {
            $(".working_pharmacy_form").attr(
                "action",
                base_url + "/update_pharmacy_timeslot"
            );
            $("input[name=working_id]").val(result.data.id);
            if (result.data.status == 1) {
                $("input[name=status]").prop("checked", true);
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == true) {
                ...
                $(".update_template").attr(
                    "action",
                    base_url + "/update_template/" + result.data.id
                );
            }
        },
        error: function (err) {},
...
function add_cancel_reason() {
    $(".cancel_reason").append(
        '<tr><td><input type="text" name="cancel_reason[]" class="form-control" required></td><td><button type="button" class="btn btn-danger removebtn"><i class="fas fa-times"></i></button></td></tr>'
    );
}
        ...
        success: function (result) {
            if (result.success == true) {
                ...
                if (result.data.length != 0) {
                    result.data.forEach((element) => {
                        $(".display").after(
                            "<tbody>" +
                                ...
                                "<td>" +
                                result.currency +
                                element.doctor_amount +
                                "</td>" +
                                "<td>" +
                                result.currency +
                                element.admin_amount +
                                "</td>" +
                                "</tbody>"
                        );
                    });
                    ...
                    $(".display").after(
                        '<tbody class="text-center"><td colspan="4">No details found</td></tbody>'
                    );
                }
            }
        },
        ...
        success: function (result) {
            if (result.success == true) {
                ...
                if (result.data.shipping_at == "home") {
                    ...
                    $(".deliveryCharge").text(
                        result.currency + result.data.delivery_charge
                    );
                } else {
        ...
        success: function (result) {
            if (result.success == true) {
                ...
                if (result.data.radiology_category == null) {
                    ...
                    $(".radiology_category").text(
                        result.data.radiology_category
                    );
                    $(".types").html("");
                    $(".types").append(
                ...
                if (result.data.pathology_category == null) {
                    ...
                    $(".pathology_category").text(
                        result.data.pathology_category
                    );
                    $(".types").html("");
                    $(".types").append(
...
function testMail() {
    ...
    if (mail_to == "") {
        ...
        $(".emailstatus").html(
            '<div class="text-success mt-3"><h6>Sending...</h6></div>'
        );
        $.ajax({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            ...
            success: function (result) {
                if (result.success == true) {
                    $(".emailstatus").html("");
                    $(".emailstatus").html(
                        ' <div class="text-success mt-3"><h6>' +
                            result.message +
                            "</h6></div>"
                    );
                    ...
                    $(".emailstatus").html("");
                    $(".emailstatus").html(
                        ' <div class="text-danger mt-3"><h6>' +
                            result.message +
                            "</h6></div>"
                    );
                    $(".emailerror").html(
                        ' <div class="text-danger mt-2"><p>' +
                            result.message +
                            "</p></div>"
                    );
                }
