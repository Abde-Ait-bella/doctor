...
$(".add-favourite").click(function () {
    ...
    if (
        $(this).find("i").hasClass("fa-regular fa-bookmark") &&
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == true) {
                ...
                if (
                    result.data.shipping_at == "home" ||
                    result.data.shipping_at == "Home"
                ) {
                    $(".shippingAddressTr").show();
                    $(".shippingAddress").text(result.data.address.address);
                    let charge =
                        result.data.delivery_charge == null
                            ? 0
                            : result.data.delivery_charge;
                ...
                result.data.medicine_name.forEach((element) => {
                    $(".tbody").append(
                        '<tr><td class="text-sm text-gray-600 px-2 py-2 text-center font-fira-sans">' +
                            element.name +
                            "</td>" +
                            '<td class="text-sm text-gray-600 px-2 py-2 text-center font-fira-sans">' +
                            element.qty +
                            "</td>" +
                            '<td class="text-sm text-gray-600 px-2 py-2 text-center font-fira-sans">' +
                            result.currency +
            ...
            if (result.success == true) {
                ...
                if (result.data.radiology_category == null) {
                    ...
                    result.data.radiology.forEach((element) => {
                        $(".types tbody").append(
                            '<tr><td class="text-sm font-light px-2 py-2 font-fira-sans">' +
                                element.screening_for +
                                "</td>" +
                                '<td class="text-sm font-light px-2 py-2 font-fira-sans">' +
                                result.currency +
                                element.charge +
                                "</td>" +
                                '<td class="text-sm font-light px-2 py-2 font-fira-sans">' +
