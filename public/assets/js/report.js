        ...
        if (value != null && value != undefined && value != "") {
            $.ajax({
                ...
                success: function (result) {
                    ...
                    amount = result.total;
                    result.data.forEach((element) => {
                        $(".pathology_single_details").append(
                            '<div class="singleDetailsDiv' +
                                element.id +
                                ' flex justify-around border border-white-light p-2 m-2"><div>' +
                        ...
                        if (element.prescription_required == 1) {
                            if ($(".presciption_required").hasClass("hidden")) {
                                ...
                                $(".presciption_required").removeClass(
                                    "hidden"
                                );
                            }
        ...
        if (this.value != null && this.value != undefined && this.value != "") {
            $.ajax({
                ...
                success: function (result) {
                    ...
                    result.data.forEach((element) => {
                        $(".radiology_single_details").append(
                            '<div class="singleRadioDiv' +
                                element.id +
                                ' flex justify-around border border-white-light p-2 m-2"><div>' +
                                '<label for="" class="form-label mb-1">Report Days</label>' +
                                '<p class="report_days">' +
            ...
            success: function (result) {
                ...
                if (result.data.length > 0) {
                    $.each(result.data, function (key, value) {
                        ...
                        $(".timeSlotRow").append(
                            '<div class="m-1 d-flex time ' +
                                select +
                                " timing" +
                                key +
                                ' rounded-3" onclick="thisTime(' +
                                key +
                                ')">' +
                    ...
                    $(".timeSlotRow").html(
                        '<strong class="text-danger text-center w-100">At this time doctor is not availabel please change the date...</strong>'
                    );
                }
            },
            error: function (err) {},
    ...
    $("#testForm").validate({
        ...
        rules: {
            ...
            pathology_category_id: {
                required: function () {
                    return (
                        $('input[name="category"]:checked').val() == "pathology"
                    );
                },
            },
            "pathology_id[]": {
            ...
            radiology_category_id: {
                required: function () {
                    return (
                        $('input[name="category"]:checked').val() == "radiology"
                    );
                },
            },
            "radiology_id[]": {
