    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == true) {
                var data =
                    '<div class="form-check"><input class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" id="address' +
                    result.data.id +
                    '" value=' +
                    result.data.id +
                    ' name="address_id"><label class="form-check-label font-fira-sans text-sm inline-block text-gray-800" for="address' +
                    result.data.id +
                    '">' +
                    result.data.address +
                    "</label></div>";
...
function addCart(id, operation) {
    ...
    if (operation == "plus") $("#minus" + id).removeClass("disabled");
    else {
        if (qty == 0) $("#minus" + id).addClass("disabled");
        else $("#minus" + id).removeClass("disabled");
    }
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success == true) {
                ...
                if (
                    result.data.cartString != "" &&
                    result.data.cartString != undefined
                ) {
                    $(".sessionCart" + id).html("");
