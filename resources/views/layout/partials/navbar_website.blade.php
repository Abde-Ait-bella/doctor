        ...
        <div class="flex flex-wrap items-center justify-between mx-auto">
            ...
            <div class="flex items-center md:order-2">
                ...
                <button type="button" data-dropdown-toggle="language-dropdown-menu"
                    class="inline-flex items-center justify-center px-4 py-2 text-sm text-gray-500 rounded-lg cursor-pointer">
                    <img src="{{ asset('images/upload/' . $lang_image) }}" class="w-5 h-5 mr-2 rounded-full"
                        alt="">
                    {{ $lang_name }}
                </button>

                <div class="z-50 hidden my-4 text-base list-none divide-y divide-gray-100 rounded-lg shadow bg-white"
                    id="language-dropdown-menu">
                    <ul class="py-2" role="none">
                        @foreach ($languages as $language)
                            <li>
                                <a href="{{ url('select_language', $language->id) }}"
                                    class="block px-4 py-2 text-sm text-black-700" role="menuitem">
                                    <div class="inline-flex items-center">
                                        <img alt="Image placeholder"
                                            src=" {{ asset('images/upload/' . $language->image) }}"
                ...
                <div class="flex items-center relative">
                    <div class="my-auto cart">
                        <a class="text-gray-500 hover:text-gray-700 focus:text-gray-700 mr-4 flex relative"
                            ...
                            <p class="absolute flex items-center justify-center tot_cart">
                    ...
                    @else
                        <div class="mt-auto mb-auto ml-3 xxsm:ml-0">
                            <a href="{{ url('/patient-login') }}"
                                class="rounded-none bg-primary tracking-wide px-4 py-2 text-white font-fira-sans font-normal text-sm">{{ __('Sign In') }}</a>
                        </div>
                    @endif
                </div>
                ...
                <ul
                    ...
                    <li>
                        <a href="{{ url('/show-doctors') }}"
                            class="{{ $activePage == 'doctors' ? 'text-primary' : 'text-black' }} font-fira-sans block py-2 pl-3 pr-4 rounded md:bg-transparent md:p-0"
