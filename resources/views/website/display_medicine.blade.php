@if (count($medicines) > 0)
    ...
    @foreach ($data as $value)
        <div class="bg-white-50 displayMedicine p-5 border border-white-light hover:drop-shadow-lg hover:border-none">
            ...
            <p class="text-slate-500 text-left font-medium text-lg text-black py-2 leading-5 font-fira-sans">
                {{ $value['name'] }}</p>
            <div class="flex justify-between">
                <p class="font-fira-sans font-medium text-xl leading-6 text-primary text-left">
                    {{ $currency }}{{ $value['price_pr_strip'] }}</p>
