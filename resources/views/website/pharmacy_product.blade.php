...
@section('content')
    <div class="xl:w-3/4 mx-auto">
        ...
        <div
            class="flex pt-5 2xl:flex-row xl:flex-row xlg:flex-row lg:flex-row xmd:flex-rowmd:flex-row sm:flex-row xsm:flex-col xxsm:flex-col">
            <div class="2xl:w-1/4 xl:w-1/4 xlg:w-80 lg:w-1/4 sm:w-96  px-4 py-5">
                <div class="flex justify-center">
                    <div class="mb-3 xl:w-96">
                        <div class="flex">
                            <div class="relative w-full">
                                <input type="search" id="search-dropdown" name="medicine_name"
                                    class="block p-2.5 w-full z-20 text-sm text-gray-900 font-fira-sans border-[#D8D8D8]"
                                    placeholder="Search" required>
                                <button type="button" onclick="medicineSearch()"
                                    class="btn absolute top-0 right-0 p-2.5 text-sm font-medium text-white bg-blue-700 border font-fira-sans">
                ...
                <form id="filter_form" class="mt-3" method="post">
                    <div>
                        <h1 class="font-fira-sans font-md text-base leading-5 font-medium mb-2 text-black-dark uppercase ">
                            {{ __('Categories') }}</h1>
                        @foreach ($categories as $category)
                            <div class="form-check p-1">
                                <input name="select_specialist"
                                    class="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm bg-white-50 checked:bg-primary checked:border-primary focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"
                                    type="checkbox" value="{{ $category->id }}" id="category{{ $category->id }}">
                                <label class="font-fira-sans form-check-label inline-block text-gray-800"
