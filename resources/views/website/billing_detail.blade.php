...
@section('content')
    ...
    <div class="xl:w-3/4 mx-auto">
        <div class="xxsm:mx-5 xl:mx-0 2xl:mx-0 pt-10 pb-20">
            ...
            <div
                class="flex w-full 2xl:mx-0 xl:mx-5 2xl:flex-row xl:flex-row xlg:flex-row lg:flex-row xmd:flex-row md:flex-row sm:flex-col xsm:flex-col xxsm:flex-col gap-10">
                <div
                    class="border border-white-light p-5 2xl:w-3/4 xl:w-3/4 xlg:w-3/4 lg:w-3/4 xmd:w-3/4 md:w-3/4 sm:w-3/4 xsm:w-full xxsm:w-full mb-10">
                    <h1 class="font-fira-sans leading-6 text-xl font-medium pb-4">{{ __('Personal Details ') }}</h1>
                    <div class="form-check">
                        <input id="delivery_type"
                            class="form-check-input appearance-none h-4 w-4 border border-gray-300 rounded-sm checked:bg-primary checked:border-primary focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"
                    ...
                    <div>
                        <div class="border border-gray-light rounded-2 mt-4 hidden addresses-list px-3 pb-3">
                            <div
                                class="flex justify-between border-b border-gray-light p-3 align-items-center shipping-details mb-2">
                                <h3 class="location-name font-fira-sans">{{ __('Shipping Details') }}</h3>
                                <div class="success-block w-auto">
                                    <a class="font-fira-sans text-sm text-primary" href="javascript:void(0)"
                                        data-te-toggle="modal"
                                        data-te-target="#exampleModalCenter">{{ __('Add new Address') }}</a>
                                </div>
                            </div>
                            ...
                            <div class="flex">
                                <div class="displayAddress">
                                    @foreach ($master['address'] as $address)
                                        <div class="form-check">
                                            <input
                                                class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 checked:bg-primary checked:border-primary focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"
                                                type="radio" id="address{{ $address['id'] }}"
                                                value="{{ $address['id'] }}" name="address_id">
                                            <label
                                                class="form-check-label font-fira-sans text-sm inline-block text-gray-800"
                    ...
                    <div class="px-2 py-2  {{ $master['prescription'] == 0 ? 'hidden' : '' }}">
                        <p class="mb-1 mt-3 text-sm font-fira-sans prescription-txt">{{ __('Add Prescription PDF') }} <span
                                class="text-red">{{ __('**Please First Add Doctor Prescription**') }}</span></p>
                        <input type="file" name="pdf" accept=".pdf" id="pdf"
                            class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray bg-clip-padding border
                 border-gray-light rounded transition ease-in-out m-0 focus:text-gray-700 focus focus:border-primary focus:outline-none">
                    ...
                    <div class="payment_style {{ $master['prescription'] == 1 ? 'hidden' : '' }}">
                        ...
                        <div class="flex xxsm:flex-col lg:flex-row mb-10 gap-10">
                            @if ($setting->cod)
                                <div class="border border-white-100 font-fira-sans paymentDiv text-center activePayment"
                                    data-attribute="cod">
                            ...
                            @endif
                            @if ($setting->paypal)
                                <div class="border border-white-100 font-fira-sans paymentDiv text-center"
                        ...
                        <div class="codDiv text-center">
                            <input type="button"
                                ...
                                onclick="bookMedicine()" value="{{ __('Pay Cash on Delivery') }}">
                        </div>
                                    ...
                                    <div class="flex" class="mt-1">
                                        <div class="w-1/2">
                                            <input type="text"
                                                ...
                                                pattern="(0[1-9]|10|11|12)/[0-9]{2}$" placeholder="MM/YY" required />
                                        ...
                                        <div class="w-1/2">
                                            <input type="text"
                                                ...
                                                title="please input only number." pattern="[0-9]{3}" name="card-cvc"
                        ...
                        <div class="flutterwaveDiv hidden">
                            <form>
                                ...
                                <div class="w-full px-4 flex gap-3 items-center mt-5 rounded-md h-auto justify-center">
                                    <input type="button"
                                        class="font-fira-sans text-white bg-primary p-3 text-sm font-normal py-3 cursor-pointer"
                                        onclick="makePayment(event)" value="{{ __('Pay With Flutterwave') }}">
                                </div>
                            </form>
                        </div>
                ...
                <div class="p-8" style="background: #f4fbfd;">
                    ...
                    <table
                        class="w-full text-sm text-left text-gray-500 dark:text-gray-400 border-b border-white-light pb-7">
                        ...
                        <tbody class="">
                            @foreach (Session::get('cart') as $cart)
                                <tr class="border-b border-white-light">
                                    <th scope="row"
                                        class="flex 2xl:flex-row  xl:flex-col xlg:flex-col lg:flex-col xmd:flex-col  md:flex-col sm:flex-row msm:flex-row  xxsm:flex-col items-center py-4  text-gray-900 whitespace-nowrap dark:text-white">
                    ...
                    <div class="flex justify-between border-b border-white-light py-2">
                        ...
                        <div>
                            <h1 class="font-fira-sans text-sm font-medium leading-5 text-black">
                                {{ $setting->currency_symbol }}<span class="subtotal">{{ $price }}</span></h1>
                        </div>
                    </div>
                    <div class="flex justify-between border-b border-white-light py-2">
                        <div>
                        ...
                        <div>
                            <h1 class="font-fira-sans text-sm font-medium leading-5 text-black">
                                {{ $setting->currency_symbol }}<span class="deliveryCharge">00</span></h1>
                        </div>
                    </div>
                    <div class="flex justify-between pt-2 pb-8">
                        ...
                        <div>
                            <h1 class="font-fira-sans text-sm font-medium leading-5 text-black">
                                {{ $setting->currency_symbol }}<span class="finalPrice">{{ $price }}</span></h1>
                        </div>
                    </div>

                    <button type="button" onclick="bookMedicine()"
                        class="w-full text-white bg-primary text-center px-4 py-2 text-base font-normal leading-5 font-fira-sans ">{{ __('CheckOut') }}</button>
                </div>
            </div>
    ...
    <div data-te-modal-init
        ...
        id="exampleModalCenter" tabindex="-1" aria-labelledby="exampleModalCenterLabel" aria-hidden="true">
            ...
            <div
                ...
                <div
                    ...
                    <h5 class="text-xl font-medium leading-normal text-neutral-800 dark:text-neutral-200"
                        id="exampleModalCenterLabel"> {{ __('User Address') }}</h5>
                    <button type="button"
                ...
                <div class="relative overflow-y-auto p-4">
                    <form class="addAddress" method="post">
                        <div class="w-auto border border-gray-light" id="map" style="height: 200px">
                            {{ __('Rajkot') }}</div>
                        <input type="hidden" name="lat" id="lat" value="{{ $setting->lat }}">
                        <input type="hidden" name="lang" id="lng" value="{{ $setting->lang }}">
                        <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
                        <textarea name="address"
                            class="mt-2 form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus focus:border-primary focus:outline-none"
                            id="exampleFormControlTextarea1" rows="3">{{ __('Rajkot,Gujrat') }}</textarea>
                    </form>
                </div>
                <div
