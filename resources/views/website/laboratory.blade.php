...
@section('content')
    ...
    <div class="pt-14 border-b border-gray-light mb-10 pb-10">
        ...
        <div class="p-5">
            <p class="font-fira-sans font-normal text-lg text-center leading-5 text-gray">
                {{ __('Book Your Appointment with Easy Way') }}</p>
        </div>
        <form id="searchForm" method="post">
            <div
                class="flex justify-center  2xl:flex-row xl:flex-row xlg:flex-row lg:flex-row xmd:flex-row md:flex-row sm:flex-row msm:flex-col xsm:flex-col xxsm:flex-col space-x-5 xmd:space-y-0 sm:space-y-0 sm:space-x-5 msm:space-x-0 xsm:space-x-0 xxsm:space-x-0 msm:p-5 msm:space-y-2 xsm:space-y-2 xsm:p-5 xxsm:space-y-2 xxsm:p-2">
                <div class="relative">
                    <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                        <i class="fa-solid fa-magnifying-glass"></i>
                    </div>
                    <input type="search" name="search_val" id="default-search"
                        class="block p-2 pl-10 text-sm border border-white-light 2xl:w-96 xmd:w-72 sm:w-40 h-12"
                        placeholder="{{ __('Search Laboratory...') }}" required>
                </div>
                <div class="relative">
                    <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                        <i class="fa-solid fa-location-dot"></i>
                    </div>
                    <input type="search" name="address" id="autocomplete" onFocus="geolocate()"
                        class="block p-2 pl-10 text-sm border border-white-light 2xl:w-96 xmd:w-72 sm:w-40 h-12"
                        placeholder="{{ __('Set your location') }}" required>
                    <input type="hidden" name="lab_lat">
                    <input type="hidden" name="lab_lang">
                </div>
                <input type="hidden" name="from" value="js">
                <button type="button" onclick="labSearch()"
    ...
    <div class="xl:w-3/4 mx-auto mb-16">
        <div
            class="flex pt-5 2xl:flex-row xl:flex-row xlg:flex-row lg:flex-row xmd:flex-row md:flex-row sm:flex-row xsm:flex-col xxsm:flex-col xxsm:mx-5 xl:mx-0 2xl:mx-0">
            <div class="2xl:w-1/4 xl:w-1/4 xlg:w-1/4 lg:w-1/4 sm:w-60 px-4 space-y-7 py-5">
                <form id="filter_form">
                    <div>
                        <h1 class="font-fira-sans font-medium text-base leading-5 text-black-dark">{{ __('Categories') }}
                        </h1>
                        <div class="form-check p-1">
                            <input
                                class="form-check-input appearance-none h-4 w-4 border border-gray-light rounded-sm bg-white-50 checked:bg-primary checked:border-primary focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"
                                name="select_specialist" type="checkbox" value="latest" id="latest">
                            <label
                                class="form-check-label font-fira-sans font-normal text-sm leading-4 inline-block text-black"
                                for="latest"> {{ __('Latest') }}
                            </label>
                        </div>
                        <div class="form-check p-1">
                            <input
                                class="form-check-input appearance-none h-4 w-4 border border-gray-light rounded-sm bg-white-50 checked:bg-primary checked:border-primary focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"
                                name="select_specialist" type="checkbox" value="availability" id="availability">
                            <label
                                class="form-check-label font-fira-sans font-normal text-sm leading-4 inline-block text-black"
                                for="availability">
                                {{ __('Availability') }}
            ...
            @if ($labs['current_page'] != $labs['last_page'])
                <div
                    ...
                    <div class="sm:py-3 md:py-0 msm:py-3 xsm:py-3 xxsm:py-3" id="">
                        <button id="more-doctor" type="button"
