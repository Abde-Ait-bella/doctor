    ...
    <div class="pt-14 border-b border-white-light mb-10 pb-10">
        ...
        <div class="p-5">
            <p class="font-fira-sans font-normal text-lg text-center leading-5 text-gray">
                {{ __('Lorem ipsum dolor sit amet,ng elit.') }}</p>
        </div>
            ...
            xmd:space-y-0 sm:space-y-0 sm:space-x-5 msm:space-x-0 xsm:space-x-0 xxsm:space-x-0 msm:p-5 msm:space-y-2 xsm:space-y-2 xsm:p-5 xxsm:space-y-2 xxsm:p-2">
                <div class="relative">
                    ...
                    <input type="search" name="search_val" id="default-search"
                        ...
                        placeholder="{{ __('Search Blog...') }}" required>
                </div>
                <input type="hidden" name="from" value="js">
                <button type="button" onclick="searchBlog()"
                    class="text-white bg-primary text-center px-6 py-2 text-base font-normal leading-5 font-fira-sans sm:w-32 msm:w-32 xsm:w-32 xxsm:w-32 h-12">
                    <i class="fa-solid fa-magnifying-glass"></i> {{ __('Search') }}</button>
                <a href=""
                    class="text-white bg-primary text-center px-4 py-4 text-base font-normal leading-5 font-fira-sans sm:w-32 msm:w-32 xsm:w-32 xxsm:w-32 h-12">
                    {{ __('Clear Search') }}</a>
            </div>
        </form>
    </div>
                ...
                @foreach ($blogs as $blog)
                    <a href="{{ url('blog-details/' . $blog->id . '/' . Str::slug($blog->title)) }}">
                        <div class="md:mt-5 sm:mt-11 msm:mt-11 xsm:mt-11 xxsm:mt-11 w-full">
                            <img class="lg:h-60 lg:w-full bg-cover object-cover" src="{{ asset($blog->fullImage) }}"
                                alt="" />
                            ...
                            <div
                                class="leading-4 font-fira-sans font-normal text-sm text-gray text-left h-28 overflow-hidden mt-2">
                                {{ strip_tags(html_entity_decode($blog->desc)) }}</div>
                        </div>
                    </a>
                @endforeach
            </div>
