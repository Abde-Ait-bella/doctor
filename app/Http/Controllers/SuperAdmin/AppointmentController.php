...
class AppointmentController extends Controller
{
    ...
    public function appointment()
    {
        ...
        $currency = Setting::first()->currency_symbol;
        if (auth()->user()->hasRole('super admin')) {
            $appointments = Appointment::with(['doctor', 'address'])->orderBy('id', 'DESC')->paginate(6);
        } else {
            $doctor = Doctor::where('user_id', auth()->user()->id)->first();
            $appointments = Appointment::with(['doctor', 'address', 'hospital'])->where('doctor_id', $doctor->id)->orderBy('id', 'DESC')->paginate(6);
    ...
    public function prescriptionListe()
    {
        $doctor = Doctor::where('user_id', auth()->user()->id)->first();
        $prescriptions = prescription::with(['appointment', 'user', 'doctor'])->where('doctor_id', $doctor->id)->orderBy('id', 'DESC')->paginate(6);

        return view('superAdmin.doctor.prescriptionListe', compact('prescriptions'));
    }
    ...
    public function prescription()
    {
        (new CustomController)->cancel_max_order();
        $doctor = Doctor::where('user_id', auth()->user()->id)->first();
        $appointment = Appointment::where('doctor_id', $doctor['id'])->with(['doctor', 'user'])->get();
        $medicines = Medicine::all();
        $categories = MedicineCategory::all();
        return view('superAdmin.doctor.prescription', compact('appointment', 'medicines', 'categories'));
    }
    ...
    {
        $categories = MedicineCategory::all();
        return view('superAdmin.doctor.add_medicines', compact('categories'));
    }
    ...
    public function add_medicine(Request $request)
    {
        ...
        return response(['success' => true, 'name' => $data['name'], 'id' => $last_medicine['id']]);
    }
    ...
    public function addPrescription(Request $request)
    {
        ...
        $appointment = appointment::where('id', $data['appointment_id'])->get();
        foreach ($appointment as $appointmen) {
            if (empty($appointmen['user_id'])) {
                $user_id = null;
            } else {
                $user_id = $appointmen['user_id'];
            }
        }
        ...
        for ($i = 0; $i < count($data['medicines_id']); $i++) {
            $temp['medicines_id'] = $data['medicines_id'][$i];
            $temp['dosage'] = $data['dosages'][$i];
            $temp['dose_duration'] = $data['dose_durations'][$i];
            $temp['time'] = $data['times'][$i];
            $temp['dose_interval'] = $data['dose_intervals'][$i];
            $temp['comment'] = $data['comments'][$i];
            array_push($medicine, $temp);
        }
        $data['medicines'] = json_encode($medicine);

        echo '<pre>';
        print_r(count($data['medicines_id']));
        echo '</pre>';


        $data['next_visit'] = $data['number_period'] . " " . $data['type_period'];
        ...
        return redirect('prescriptionListe');
    }
    ...
    public function editPrescription($id)
    {
        ...
        $medicinesPresc = json_decode($prescription['medicines'], true);
        ...
        return view('superAdmin.doctor.edit_prescription', compact('prescription', 'appointment', 'medicinesPresc', 'medicines', 'medicine', 'categories'));
    }
    ...
    {
        $prescription = Prescription::with(['appointment', 'doctor'])->find($id);
        $doctor = Doctor::with(['hospital'])->find($prescription->doctor->id);
        ...
        return view('superAdmin.doctor.prescriptionDetails', compact('prescription', 'doctor', 'medicinesPresc', 'category', 'medicine'));
    }
